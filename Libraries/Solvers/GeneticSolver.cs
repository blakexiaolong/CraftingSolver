...
//         {
//             return Atlas.Actions.ProgressActions.Contains(actions.Last().ID);
//         }
...
//         {
//             return actions.Count < 2 || !(actions[0].Equals(actions[1]) && Atlas.Actions.Buffs.Contains(actions[0].ID));
//         }
//         public static bool AuditFirstRound(List<Action?> actions, List<Action> crafterActions)
//         {
//             return !Atlas.Actions.FirstRoundActions.Any(x => actions.LastIndexOf(Atlas.Actions.AllActions[x]) > 0);
//         }
...
//             int generation = 1;
//
//             genes = _sim.Crafter.Actions.Select(x => Atlas.Actions.AllActions[x]).ToArray();
