    ...
    class JABOASolver2
    {
        ...
        private DfsState GenerateProgressTreeInner(ActionNode node, Action action, double prevScore, int remainingDepth, int cpLimit)
        {
            ...
            if (state.Success(_sim))
            {
                ...
                List<Action> path = node.GetPath(state.Value.Step);
                _scoredProgressLists.Add((int)ListToCpCost(path), path);
            }
        ...
        #region Quality Tree
        private void GenerateQualityTree()
        {
        ...
        private bool InlineMerger(List<Action> qualityList)
        {
            for (int i = 0; i < _progressLists.Count; i++)
            {
                ...
                if (cpMax - ListToCpCost(qualityList, false) - ListToCpCost(progressList, false) - MinDurabilityCPCost(qualityList, cpMax) < 0) continue;
                ...
                if (Atlas.Actions.FirstRoundActions.Contains(progressList[0])) progressMerger = (uint)Math.Pow(2, progressLength - 1) + 1;
